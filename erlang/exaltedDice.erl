...
-module( exaltedDice ).
-export( [averageSuccesses/2, rollGroup/2, rollRange/1, numMoreThan/2] ).
%-import( [random, lists] ).
...
% We ARE doing exalted, but for mortals.
%      Roll == 10 ->
...
numMoreThan( Lst, Targetnum ) ->
   Total = fun( Elem, AccIn ) -> 
         if
            Elem >= Targetnum -> AccIn + 1;
            true -> AccIn
         end
   end,
   lists:foldl( Total, 0, Lst ).
