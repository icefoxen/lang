...
(defun search-tree (x tree)
   (cond
      ((null tree) nil)  ;; the tree is empty
      ((= x (value tree)) tree)  ;; the root node is = to the value
      ((< x (value tree)) ;; The value is < than the current value, so search
            (search-tree x (left-subtree tree)))  ;; the left subtree.
      (t (search-tree x (right-subtree tree)))
